...
import { depositRequests as initialRequestsData, transactions } from '@/lib/data';
import { useUserStore } from '@/lib/user-store';


export function DepositRequestTable() {
    const [requests, setRequests] = useState(initialRequestsData.filter(r => r.status === 'Pending'));
    const { toast } = useToast();
    const { creditUser } = useUserStore();
    ...
    const handleRequest = (id: string, status: 'Approved' | 'Rejected') => {
        ...
        if (status === 'Approved') {
            creditUser(requestToUpdate.userId, requestToUpdate.amount);
            transactions.push({
        ...
        toast({
            ...
            description: `Deposit request ${id} has been ${status}.`,
            variant: status === 'Rejected' ? 'destructive' : 'default',
        });
    };

    if (requests.length === 0) {
        return <p className="text-center text-muted-foreground p-8">No pending deposit requests.</p>
    }
